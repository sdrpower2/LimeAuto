...
class Lime_Auto_GUI(gr.top_block, Qt.QWidget):
    ...
    def set_fc_slider(self, fc_slider):
        ...
        self.limesdr_source_0.set_center_freq(self.fc_slider, 0)


def argument_parser():
    parser = OptionParser(usage="%prog: [options]", option_class=eng_option)
    parser.add_option(
        "-f", "--fc", dest="fc", type="eng_float", default=eng_notation.num_to_str(393.525e6),
        help="Set Frequency to analyze [default=%default]")
    return parser


def main(top_block_cls=Lime_Auto_GUI, options=None):
    if options is None:
    ...
    tb = top_block_cls(fc=options.fc)
    tb.start()
    tb.show()
